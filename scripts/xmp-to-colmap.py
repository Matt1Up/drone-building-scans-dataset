...
# Image extension case varies by source: RealityScan exports .jpg, DJI aircraft write .JPG.
# Resolve against what is actually on disk rather than assuming one.
_INDEX = {}
def resolve(stem):
    if not _INDEX:
        for f in os.listdir(img_dir):
            _INDEX[os.path.splitext(f)[0]] = f
...
for p in sorted(glob.glob(os.path.join(xmp_dir, "*.xmp"))):
    ...
    fname = resolve(base)
    if fname is None:
        skipped.append(os.path.basename(p) + " (no matching image)"); continue
    cams.append(dict(name=fname, R=R, C=C,
